...
def vote_image(nnf, source_style, patch_size):
    """
    Plain-average voting — the real imaging path. A target pixel q is covered by
    patch_size² patches; the patch centered at p = q - d claims q looks like style
    pixel nnf[p] + d. Average every claim.
...
if __name__ == "__main__":
    ...
    # 1. Math check: the identity NNF (every pixel maps to itself, target == source)
    ...
    # 2. Random NNF: bounds invariant + visual debug output
